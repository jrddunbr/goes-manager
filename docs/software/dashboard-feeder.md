...
- Serve map data in friendly dashboard with a live feed of updates on the right and a map on the left.
...
## Outputs
- JSON/GeoJSON payloads for web clients with API endpoints targeted for browser consumption.
- Websocket updates pushing per-second changes with a live feed of data.
